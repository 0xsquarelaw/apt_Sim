...
const Sidebar: React.FC<SidebarProps> = ({ collapsed, toggleSidebar }) => {
  const navItems = [
    ...
    { name: 'Threat Simulation', icon: <Shield size={20} />, path: '/threat-simulation' },
    ...
    <div className={`h-screen bg-gray-900 text-white transition-all duration-300 ${collapsed ? 'w-16' : 'w-64'}`}>
      <div className="flex items-center justify-between p-4 border-b border-gray-700">
        ...
        <button 
          ...
          className={`text-gray-400 hover:text-white ${collapsed ? 'mx-auto' : ''}`}
        >
          {collapsed ? '→' : '←'}
        </button>
      </div>
      ...
      <div className="absolute bottom-0 w-full border-t border-gray-700 p-4">
        <button className={`flex items-center text-gray-400 hover:text-white ${collapsed ? 'justify-center' : 'justify-start'}`}>
          <LogOut size={20} />
